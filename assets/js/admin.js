...
jQuery(document).ready(function($) {
    ...
    if (typeof ocaiData === 'undefined') return;

    var ajaxurl = ocaiData.ajaxurl;
    var nonce = ocaiData.nonce;
            ...
            if (response.success) {
                ...
                if (response.data.length === 0) {
                    ...
                    response.data.forEach(function(item) {
                        ...
                        html += '<input type="checkbox" name="ocai_target_cats[]" value="' + item.id + '"> ' + item.name;
                        html += '</label>';
                    });
                }
                container.html(html);
    ...
    $('#ocai_target_cpt').on('change', function() {
        load_taxonomies($(this).val());
    });

    // Carregamento Inicial
    if ($('#ocai_target_cpt').length > 0) {
        load_taxonomies($('#ocai_target_cpt').val());
    }
    ...
    $('.gan-run-now').on('click', function() {
        ...
        $.post(ajaxurl, { action: 'ocai_process_instant', item_id: id, _ajax_nonce: nonce }, function(response) {
            if (response.success) {
                location.reload();
            } else {
    ...
    $('.gan-delete').on('click', function() {
        ...
        var id = $(this).data('id');
        $.post(ajaxurl, { action: 'ocai_delete_item', item_id: id, _ajax_nonce: nonce }, function() {
            $('#row-' + id).fadeOut();
        });
    });
});
