...
class Keywords {
    public function render_page() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'ocai_queue';

        \OmniContentAI\Core\Installer::install();

            if (isset($_POST['ocai_add_keywords']) && check_admin_referer('ocai_add_kw_action')) {
            $raw_kw = explode("\n", sanitize_textarea_field($_POST['ocai_keywords_list']));
            $niche = sanitize_text_field($_POST['ocai_target_niche'] ?? 'general');
            $intent = sanitize_text_field($_POST['ocai_target_intent'] ?? 'informational');
            $post_type = sanitize_text_field($_POST['ocai_target_cpt'] ?? 'post');
            $cats = isset($_POST['ocai_target_cats']) ? implode(',', array_map('intval', $_POST['ocai_target_cats'])) : '';
            ...
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Lote alocado com sucesso!', 'omnicontent-ai') . '</p></div>';
        }
        ...
        <div class="wrap gan-wrap" style="max-width: 1100px; margin-top:30px;">
            <div style="background:#fff; padding:40px; border-radius:15px; box-shadow:0 10px 30px rgba(0,0,0,0.05);">
                <h1 style="font-weight:800; margin-bottom:30px;">🛰️ OmniContent AI: Dashboard de Produção</h1>

                <form method="post" style="background:#f9f9f9; padding:25px; border-radius:12px; border:1px solid #eee; margin-bottom:30px;">
                    <?php wp_nonce_field('ocai_add_kw_action'); ?>
                    
                    <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:20px; margin-bottom:20px;">
                        <!-- Post Type -->
                        <div>
                            <label style="font-weight:700; display:block; margin-bottom:8px; color:#2271b1;">🏛️ <?php esc_html_e('Destino (CPT):', 'omnicontent-ai'); ?></label>
                            <select name="ocai_target_cpt" id="ocai_target_cpt" style="width:100%; height:45px; border-radius:8px; border:1px solid #ddd;">
                        ...
                        <div>
                            ...
                            <select name="gan_target_niche" style="width:100%; height:45px; border-radius:8px; border:1px solid #ddd;">
                                <option value="expert_authority">💎 Autoridade Máxima (Elite)</option>
                                <option value="product_comparison">⚖️ Comparativo de Produtos</option>
                                <option value="local_expert">📍 SEO Local / Regional</option>
                                <option value="educational_tutorial">📖 Guia Educativo / DIY</option>
                                <option value="recipe">🥘 Receitas</option>
                                <option value="news">📰 Notícias</option>
                                <option value="general">🌍 Geral / Universal</option>
                            </select>
                        </div>

                        <!-- Intenção de Busca -->
                        <div>
                            <label style="font-weight:700; display:block; margin-bottom:8px; color:#10b981;">⚡ <?php esc_html_e('Intenção:', 'generator-ai-now'); ?></label>
                            <select name="gan_target_intent" style="width:100%; height:45px; border-radius:8px; border:1px solid #ddd;">
                                <option value="informational">📚 Informacional (Aprender)</option>
                                <option value="commercial">💰 Comercial (Pesquisar p/ Comprar)</option>
                                <option value="transactional">🛒 Transacional (Ação/Compra)</option>
                                <option value="navigational">🧭 Navegacional (Marca)</option>
                            </select>
                        </div>
                        ...
                        <div style="position:relative;">
                            ...
                            <details style="position:relative; width:100%;">
                                <summary style="height:45px; display:flex; align-items:center; padding:0 15px; background:#fff; border:1px solid #ddd; border-radius:8px; cursor:pointer; list-style:none;">
                                    <?php esc_html_e('Selecionar...', 'generator-ai-now'); ?>
                    ...
                    <textarea name="gan_keywords_list" style="width:100%; height:100px; border-radius:10px; border:1px solid #ddd; padding:15px; font-size:14px;" placeholder="Uma palavra-chave por linha..."></textarea>
                    
                    <button type="submit" name="gan_add_keywords" class="button button-primary" style="margin-top:20px; height:50px; padding:0 40px; border-radius:30px; font-weight:700; background:#ec5b13; border:none;"><?php esc_html_e('Alocar na Fila de Produção', 'generator-ai-now'); ?></button>
                </form>
                    ...
                    <tbody>
                        ...
                        <tr id="row-<?php echo esc_attr($item->id); ?>">
                            ...
                            <td>
                                ...
                                <div style="font-size:10px; color:#666; margin-bottom:5px;">✨ <?php echo esc_html(ucfirst($item->niche)); ?> (<?php echo esc_html(ucfirst($item->intent)); ?>)</div>
