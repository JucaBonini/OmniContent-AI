...
class Ajax {
    public function __construct() {
        add_action('wp_ajax_ocai_process_instant', [$this, 'process_instant_item']);
        add_action('wp_ajax_ocai_delete_item', [$this, 'delete_item']);
        add_action('wp_ajax_ocai_move_item', [$this, 'move_item']);
        add_action('wp_ajax_ocai_get_taxonomies', [$this, 'get_taxonomies_for_cpt']);
    }

    public function get_taxonomies_for_cpt() {
        check_ajax_referer('ocai_queue_nonce');
    ...
    public function delete_item() {
        ...
        global $wpdb;
        $wpdb->delete($wpdb->prefix . 'ocai_queue', ['id' => (int) $_POST['item_id']]);
    ...
    public function process_instant_item() {
        ...
        if ($item && ($item->status === 'pending' || $item->status === 'error')) {
            ...
            $result = $generator->generate_content($item->keyword, $item->niche, $item->intent);
